...
(async()=>{let browser;const results=[];try{
 ...
 await page.route('**/game/app.js',route=>{const source=readFileSync('dist/lab/game/app.js','utf8').replace('function prepareLabBase(', 'window.__expireLabRound = function () { state.roundElapsed = 90; state.endReason = \"time-up\"; };\n  function prepareLabBase(');return route.fulfill({contentType:'application/javascript',body:source});});
 ...
 await page.evaluate(()=>document.getElementById('gameFrame').contentWindow.__expireLabRound());await prepare('perfect-score');const resetState=await page.evaluate(()=>document.getElementById('gameFrame').contentWindow.FOURCAST.getState());assert.ok(resetState.roundElapsed<1);assert.equal(resetState.phase,'running');results.push('Preparing a new Lab scenario resets an expired round clock');
